...
{
  private Vector oProlog;            //Collection of prolog nodes.
  ...
  * @return The Vector of prolog nodes.
  * @see com.room4me.xml.Node
  */
  public Vector getPrologNodes()
  {
    return oProlog;
  }
  ...
  */
  public SmallXMLParser(String sXML) throws MalformedXMLException
  {
    ...
    oProlog = new Vector();  //Initialize it to an empty list.
    ...
    while(bMore)
    {
      ...
      if(bEmpty)
      {
        ...
        if(!sNodeName.equals("/" + oNode.getName()))
        {
          //Report an error!
  ...
  */
  public String getXMLAsText()
  {
    ...
    for(Enumeration i=oProlog.elements();i.hasMoreElements();)
    {
      sProlog.append(getNodeXML(((Node) i.nextElement()),""));
    }
  ...
  */
  private String walkTree(Node oNode)
  {
    ...
    if(oNode.oChild == null)
    {
      ...
      Vector oChildren = oNode.getChildNodes();
      for(int i=0;i<oChildren.size();i++)
      {
        sOut.append(walkTree((Node) oChildren.elementAt(i)));
      }
